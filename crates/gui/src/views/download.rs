...
const DOWNLOAD_HELP: &[crate::widgets::tab_help::HelpBlock] = &[
    ...
    crate::widgets::tab_help::HelpBlock::B(&[
        "Кнопка «📅 Интервал» — стандартный интервал: выберите год, вкладку (Неделя/Месяц/Квартал/Год) и значение одним кликом.",
    ...
    crate::widgets::tab_help::HelpBlock::T("Под строкой отчёта — описание и тип периода. Для месячного отчёта за квартал/год выгрузки пойдут по каждому месяцу интервала автоматически."),
    ...
    crate::widgets::tab_help::HelpBlock::B(&[
        ...
        "Повторная выгрузка не создаёт дубликат — дедупликация по SHA-256.",
