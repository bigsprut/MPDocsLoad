...
const ARCHIVE_HELP: &[crate::widgets::tab_help::HelpBlock] = &[
    ...
    crate::widgets::tab_help::HelpBlock::B(&[
        ...
        "Интервал дат: кнопка «📅 Интервал» (месяц/квартал/полугодие/год) или поля «С:/По:» с календарями.",
    ...
    crate::widgets::tab_help::HelpBlock::B(&[
        ...
        "🔗 — открыть раздел отчёта в ЛК (у всех отчётов Ozon; у Wildberries ссылок нет — кнопки в строке не будет).",
        "🗑 — удалить запись и файл с диска (с подтверждением).",
    ]),
    crate::widgets::tab_help::HelpBlock::H("Колонки"),
    crate::widgets::tab_help::HelpBlock::B(&[
        "Границы колонок тянутся мышью — ширину можно подогнать под данные; ширины сохраняются при обновлении списка.",
        "Обрезанное значение ячейки — в подсказке (наведите курсор); для «Отчёта» там же технический идентификатор.",
    ]),
