...
2. ⏳ **Перенос выбора маркетплейс+профиля в Настройки + иконка/название в заголовке** — НЕ НАЧАТО.
   Решение пользователя: выбор ТОЛЬКО в Настройках (без быстрого переключателя в «Загрузке»).
   После выбора маркетплейса и профиля в заголовке программы — иконка маркетплейса и название продавца.
...
**Следующая задача — П.2** (по бэклогу секции 5):
> Перенос выбора маркетплейс+профиля в Настройки (полностью, без быстрого переключателя
> в «Загрузке») + после выбора маркетплейса и профиля в заголовке программы — иконка
> маркетплейса и название продавца.

Это крупная UI-переработка. Рекомендуется EnterPlanMode для проектирования.

**Перед П.2** желательно, чтобы пользователь:
1. Закрыл запущенный GUI (exe блокирует линковку при сборке).
2. Перевыпустил ключи Ozon/WB (старые были в открытом виде в БД).
3. Создал профили заново в GUI → секреты уйдут в keyring.
4. Подтвердил, что скачивание работает (тогда можно живые тесты API — П.3-тесты).
