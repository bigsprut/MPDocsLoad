...
- **Ближайшие задачи:** крупных не осталось. Закрыто 15.08 (ночь): test-clean-env
  + test-installed на бандле 02:06 (оба зелёные: exit 124, postinstall 0,
  loaders.cache относительные пути); живой GUI-прогон фоллбэка postings БЕЗ
  кликов (расписание fb_smoke + новый env-хук `MDWF_START_VIEW=logs` — окно
  сразу на «Журнале»; в Журнале «скачано 1 файл(ов) — внимание: Ozon не смог
  сформировать отчёт… 665 шт.», файл 44 645 байт = CLI-прогону; коммит
  ce29251). Осталось только: скриншоты ЛК для placement_* (FBO → Стоимость
  размещения) и returns — сверить последние два cabinet-пути.
