...
# Тесты
cargo test --workspace          # 164 теста
...
**В этом чате завершены**: П.2 (раздел «Магазин»), П.4 (значок «уже загружен»), + расширение
отчётов Ozon до 21, + Excel-конвертация (rust_xlsxwriter, русские колонки), + типизированная
обработка кодов возврата API (CoreError::Api). Все изменения запушены (последний коммит `646b90c`).

**Следующая задача — П.5** (по бэклогу секции 5):
> Иконка типа файла в списке документов. Небольшой объём.
> В списке документов (download.rs `render_list`) — рядом с названием показывать иконку/эмодзи
> по типу файла (xml/pdf/xlsx/zip). Расширения уже есть в `DocumentEntry.extensions`.
> Реализация: в `render_list` добавить Label/Image по первому расширению из `doc.extensions`.
> Не требует планирования — небольшая доработка `render_list`.

**После П.5 — П.6** (крупная): офлайн-режим навигации по скачанным документам с фильтрами.
Новая вкладка/функциональность: список файлов из каталога `downloads` (уже с `document_id`,
`file_path`, `downloaded_at`) с фильтрами (профиль/отчёт/период) и открытием файлов. Данные
уже есть в SQLite — нужна новая вкладка GUI + Catalog-запросы (напр. `list_downloads_filtered`).
...
4. **Выбрать магазин** в «Магазине» → проверить: заголовок (иконка + `legal_name` для Ozon),
   список отчётов, скачивание (особенно новые Excel-отчёты и значок «уже загружен»).
5. **Подтвердить, что скачивание работает** → тогда можно живые тесты API (без явного разрешения
   пользователя НЕ запускать).
