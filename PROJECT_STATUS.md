...
- **HEAD:** `9a40616` = origin/master (17.08), рабочее дерево чистое.
  Неотслеживаемые `MDWF/downloads/**` — артефакты живых прогонов (в репо
  не нужны). Выпущен релиз **v1.5.0** (2026-08-15): CHANGELOG `[1.5.0]`,
  тег + GitHub Release с `MDWFSetup-1.5.0.exe`. Ствол =
  **1.6.0-unreleased**: в CHANGELOG `[Unreleased]` накоплено ~13 пунктов
  — релиз 1.6.0 по команде юзера (политика версий: MINOR-бамп на первой
  фиче цикла уже сделан; PATCH-серии 1.6.x — только hotfix'ы ПОСЛЕ выхода
  базовой 1.6.0).
- **CI ЗЕЛЁНЫЙ** (последний зелёный run 32079574415):
  ...
  RUSTUP_TOOLCHAIN; авто-установка тулчейна не ставит clippy → явный
  `rustup toolchain install … --component clippy`). Локально clippy
  проверяется ДВУМЯ тулчейнами (1.90 + `+1.97.0-x86_64-pc-windows-gnu`).
- **Сделано чатом 16–17.08 (4fe333d…9a40616, 12 коммитов — волна
  UX-правок по жалобам юзера, каждая закрыта отдельно):**
  1. **Журнал** (`b8f7a73`): путь к файлу в сообщениях о скачивании
     (`core::journal::paths_suffix`), GUI + CLI + расписания.
  2. **Справка** (`6e4f04e`): единый TextView (выделение сквозь документ,
     «Копировать всю справку»); «О программе» — авторство/лицензии/
     trademarks/credits.
  3. **Архив** (`4fe333d`→`890ec5b`→`687a032`→`695efd7`): колонки —
     единая GtkGrid (заголовок+строки в общей сетке: 7 колонок данных +
     6 разделителей, vline/hline, hexpand у «Отчёта» в заголовке и
     данных); заголовки от левого края; разные иконки «открыть файл»
     (document-open-symbolic) / «папка» (folder-symbolic); позиция
     прокрутки сохраняется при удалении строки (vadjustment + idle).
  4. **Компактное окно** (`4fe333d`+`3c431e3`): min 840×590 (было
     ~1325×919) — диагностика measure(), обёртки ScrolledWindow против
     распирания; Настройки — 4 подвкладки Stack+StackSwitcher (Файлы/
     Сеть/Расписания/Безопасность); кнопки «Загрузки» компактно.
  5. **realization_posting Ozon** (`a07a424`): csv→xlsx как лист ЛК
     (`providers/ozon/src/xlsx.rs`: 17 колонок, типизация RpKind, «Итого»
     только по суммовым, лист 2 «Детали (API)», ДД.ММ.ГГГГ); сверен с
     файлом юзера из ЛК — итоги совпали; fallback честный csv.
  6. **Меню приложения** (`32db057`+`e8b216a`): hamburger в углу окна —
     иконка программы из gresource (from_icon_name давал белый лист —
     урок #66), рабочие «Справка»/«О программе» (setup_app_actions).
  7. **Календарь** (`a5218bc`): клик по «старому» номеру дня после смены
     месяца (quirk GtkCalendar, урок #65) + интервальный виджет
     открывается на текущем выбранном периоде (classify_range).
  8. **ЛК-ссылки Ozon** (`981986d`): по таблице юзера (21 URL
     seller.ozon.ru + 10 исправленных путей) —
     `ReportDescriptor.cabinet_url` (serde default, билдер; wb/test=None;
     guard-тест «у всех Ozon-отчётов есть URL»); в «Отчётах» link-кнопка
     на каждой строке, в «Загрузке» — «Открыть в ЛК» у подсказки режима
     (видна только при наличии URL); `open_url` через
     gio::AppInfo::launch_default_for_uri.
  9. **CI-flake фикс** (`9a40616`): `PRAGMA busy_timeout=5000` в
     `Catalog::open` ДО journal_mode (параллельные e2e-процессы падали
     «database is locked» — урок #68).
- **Зелёное состояние:** `cargo test --workspace` exit=0 (235+ тестов);
  clippy `-D warnings`=0 локально (1.90 и 1.97) и на CI.
- **Ближайшие задачи:** релиз **1.6.0** по команде юзера (CHANGELOG
  готов, пересобрать инсталлер build-setup.cmd); хвост юзера — сверка
  ЛК-путей **WB** по таблице (выдана 15.08, не проверена; для WB нет
  ссылок — только пути); опционально: проверка обновлений через GitHub
  Releases API, склейка месячных выгрузок в один xlsx.
- **Живые API-тесты разрешены пользователем** (профили `oz_prof1` +
  `wb_prof`, ключи валидны).
- Ключевые уроки нарастающим итогом: **#53-69** (новые: #65 — quirk
  GtkCalendar «старый номер дня»; #66 — обязательные изображения только
  из gresource; #67 — min-size окна и ScrolledWindow; #68 — busy_timeout
  до journal_mode; #69 — сохранение прокрутки при пересборке списка).
...
```
9a40616 fix(storage): busy_timeout при открытии каталога — гонка с journal_mode=WAL
