...
- **HEAD:** `3f43ce8` = origin/master (20.08), рабочее дерево чистое.
  Неотслеживаемые `MDWF/downloads/**` — артефакты живых прогонов (в репо
  не нужны). Выпущен релиз **v1.6.0** (2026-08-18): CHANGELOG `[1.6.0]`,
  тег + GitHub Release с `MDWFSetup-1.6.0.exe` (бандл проверен clean-env,
  схема БД мигрирует до v7). Ствол = **1.7.0-unreleased** (MINOR-бамп на
  первом изменении цикла — `ecc83f7`, политика версий в CONTRIBUTING);
  в CHANGELOG `[Unreleased]` 3 пункта.
- **CI ЗЕЛЁНЫЙ** на HEAD. Прогон включает `cargo test --workspace` +
  `cargo clippy --tests -D warnings` (windows-latest + MSYS2/mingw64;
  особенности rustup — урок #63). Локально clippy проверяется ДВУМЯ
  тулчейнами (1.90 + `+1.97.0-x86_64-pc-windows-gnu`).
- **Сделано чатом 18–20.08 (af963e5…3f43ce8, 11 коммитов):**
  1. **Откат переименования** (`af963e5`): `analytics_turnover` остаётся
     «Оборачиваемость товара» (заголовок метода в API-доке) — юзер подтвердил;
     таблица юзера синхронизирована (21/21: названия/методы/типы/пути/ссылки).
  2. **Фикс «Открыть в ЛК» на клиентских машинах** (`b007ed7`): gio-lookup
     ассоциации браузера на части машин молча фейлился; теперь нативный
     `cmd /C start "" "<url>"` (raw_arg-квотирование из-за `&` в ссылках,
     CREATE_NO_WINDOW, gio — запасной), при сбое — adw::MessageDialog с
     кнопкой «Копировать ссылку».
  3. **Кнопка «Открыть в ЛК» в Архиве** (`011dd06`): ArchiveEntry += cabinet_url
     (app-слой резолвит из дескрипторов; WB-строки кнопку не получают).
  4. **Ссылки/названия по таблице юзера** (`1ab2d6b`, `0475da8`, `d637b78`,
     `ecc83f7`): realization/realization_posting → closingDocuments/
     informationalDocuments; buyout → «Отчёт о выкупленных товарах» +
     marketplaceBuyout (без «ЕАЭС»); b2b_sales → «Продажи юрлицам» (без
     «(PDF)» — формат определяет сервер, расширение по magic bytes).
  5. **Архив — перетаскиваемые колонки** (`94cc475`): GtkColumnView вместо
     GtkGrid (set_resizable; ячейки без капа ширины — при расширении текст
     целиком; «Отчёт» эластичная; полные значения в tooltip; виджет+модель
     живут постоянно — ширины переживают refill; scroll-restore сохранён).
  6. **Журнал — кнопки действий** (`2dc2275`): записи о скачивании несут
     📂/📁/🔗 (контекст в БД: миграция v7 journal.file_path/report_type;
     пишут GUI+CLI+расписания; cabinet_url резолвится при показе).
  7. **Релиз v1.6.0** (`f75da6a`): 14 пунктов CHANGELOG, инсталлер, тег.
  8. **Различимые иконки после скачивания** (`194cb30`): папка =
     folder-symbolic (не folder-open — «стрелочные» иконки неразличимы).
  9. **Актуализация всей справки + «Период:»** (`3f43ce8`): интервалы
     месяц/квартал/полугодие/год, ЛК-кнопки, две кнопки после загрузки,
     колонки Архива, кнопки Журнала, подвкладки Настроек, FAQ про ссылку;
     подпись полей дат «Диапазон:» → «Период:».
- **Зелёное состояние:** `cargo test --workspace` exit=0 (235+ тестов);
  clippy `-D warnings`=0 локально (1.90 и 1.97) и на CI.
- **Ближайшие задачи:** (в работе) склейка месячных выгрузок в один xlsx;
  проверка обновлений через GitHub Releases API — БЛОКЕР: репо приватное,
  анонимный API отдаёт 404, нужно решение юзера (публичное репо vs токен);
  хвост юзера — сверка ЛК-путей WB по живому кабинету (чек-лист выдан,
  правок нет); опционально: укрепить Catalog::open против e2e-флэйка
  «database is locked» (был 1 раз за сессию, урок #68 residual);
  CONTRIBUTING — двусмысленная формулировка про PATCH.
- **Живые API-тесты разрешены пользователем** (профили `oz_prof1` +
  `wb_prof`, ключи валидны).
- Ключевые уроки нарастающим итогом: **#53-74** (новые: #70 — ColumnView
  API; #71 — PrintWindow/GL-окна; #72 — GDI+ PNG на D: умер, BMP-обход;
  #73 — cmd start и `&` в URL; #74 — склейка справки с UI-терминами).
...
```
3f43ce8 docs(gui): актуализация всей справки + подпись «Период:» в Загрузке
194cb30 fix(gui): различимые иконки «Открыть файл/папку» после скачивания
ecc83f7 fix(ozon): убрать «(PDF)» из названия «Продажи юрлицам» + ствол 1.7.0-unreleased
f75da6a chore(release): [Unreleased] → [1.6.0] — 2026-08-18
2dc2275 feat(gui): Журнал — кнопки действий в записях о скачивании
94cc475 feat(gui): Архив — перетаскиваемые границы колонок (GtkColumnView)
d637b78 fix(ozon): убрать «ЕАЭС» из описания отчёта о выкупленных товарах
0475da8 fix(ozon): buyout — название и ссылка по обновлённой таблице юзера
1ab2d6b fix(ozon): ссылки ЛК отчётов реализации — по обновлённой таблице юзера
011dd06 feat(gui): «Открыть в ЛК» в Архиве — кнопка в строке каждого Ozon-отчёта
b007ed7 fix(gui): «Открыть в ЛК» — нативный запуск браузера вместо gio-lookup
af963e5 revert: оборачиваемость — название как в API-доке («Оборачиваемость товара»)
c4345ae docs: PROJECT_STATUS §0+§6+§12(уроки #65-69)+итог чата 16-17.08 — хэндофф-снимок (HEAD 9a40616)
981986d feat(ozon): ссылки ЛК у отчётов + кнопки «Открыть в ЛК»
a5218bc fix(gui): календарь — клик по «старому» номеру дня после смены месяца; интервальный виджет — позиционирование на текущий период
e8b216a fix(gui): иконка приложения в меню — из gresource (from_icon_name давал белый лист)
32db057 fix(gui): меню приложения — иконка программы в углу + рабочие «Справка»/«О программе»
a07a424 feat(ozon): позаказный отчёт о реализации — xlsx вместо CSV (лист как в ЛК + детали API)
695efd7 fix(gui): Архив — разные иконки «открыть файл/папку» + позиция прокрутки при удалении
687a032 fix(gui): заголовки таблицы Архива — одинаковое выравнивание (все от левого края колонки)
890ec5b refactor(gui): таблица Архива — единая GtkGrid (заголовок+строки в одной сетке)
8cf8196 docs: CHANGELOG [Unreleased] — подвкладки настроек, компактное окно, сетка Архива
3c431e3 feat(gui): компактное окно + сетка таблицы Архива + настройки на подвкладках
4fe333d fix(gui): ровные колонки Архива + уменьшаемое окно (минимальные размеры контента)
...
0070ee5 feat(gui): --self-test — event-level headless-драйвер GUI
6bad96f docs: §0 — метаданные + релиз v1.4.0 (HEAD 0246941)
0246941 docs: метаданные проекта — LICENSE (MIT/Apache-2.0), NOTICE, CHANGELOG, SECURITY, CONTRIBUTING
f02dfb5 docs: §0 — инсталлер пересобран 10:15 (журнал-пакет + cabinet-пути), бандл проверен clean-env
b2be876 fix(ozon): realization — группы «Закрывающие»/«Справочные» в cabinet-пути (проверка юзера по живому ЛК)
4d78389 docs: §0 — returns-путь с уровнем «Моя аналитика» (HEAD 17d7050)
17d7050 fix(ozon): returns — уровень «Моя аналитика» в cabinet-пути (уточнение юзера)
6ecd8e1 docs: §0+урок #60 — все cabinet-пути сверены (HEAD d2ffcd5)
d2ffcd5 fix(ozon): последние два cabinet-пути — по базе знаний продавцов
64d1a39 docs: §0 — инсталлер 02:06 отстаёт от HEAD
8778505 docs: PROJECT_STATUS — журнал в БД с источниками, фикс CLI-исполнителя, урок #61 (HEAD 06b3b47)
06b3b47 fix(cli): исполнитель расписаний терял выгрузки + журнал и --by-task
d54f109 feat: источник события в журнале + человекочитаемые названия отчётов и периоды
9a698e9 feat(gui): журнал событий персистится в SQLite (переживает перезапуск)
26d33e2 docs: PROJECT_STATUS §0 — закрыты test-clean-env и живой GUI-прогон фоллбэка postings (HEAD ce29251)
ce29251 feat(gui): MDWF_START_VIEW — стартовая вкладка для клик-фри GUI-тестов
2ab6af9 fix(ozon): cabinet-пути — вторая партия по скриншотам ЛК (Товары/FBO/FBS/Аналитика/Баланс)
961b99d docs: PROJECT_STATUS — снимок вечера 14-15.08 (HEAD 4ea596c)
4ea596c fix(ozon): cabinet-пути — фактические названия разделов ЛК (скриншот продавца)
b0d7ab5 fix(build): build-setup.cmd из PowerShell/двойного клика — 3 починки PATH
391f8af feat: ЛК-пути отчётов в GUI + понятное информирование о фоллбэке postings
482b8c4 docs: N/A-разбор и решения зафиксированы (b2b/postings/v3/день/claims)
534509e fix(ozon): решение N/A-проблем — b2b friendly-error, postings retry+фоллбэк, /v3 fbo list, CLI-день
c57ab9e feat(gui): описание выбранного периода + вкладка «Полугодие» (Неделя убрана)
ecc28b4 feat(gui): двойной клик по карточке расписания → диалог редактирования
962d66f feat(gui): двойной клик по строке профиля → диалог редактирования
1e0eedf docs: остатки бэклога закрыты — markup/restore/stale-state фиксы, клик-тест Журнала, чистая машина
eeafba0 fix(gui): markup-скобки в справке + гонка restore комбо + stale-state guard
efcab98 fix(gui): единая иконка «Обновить» (view-refresh-symbolic) во всех вкладках
6bacad0 fix: типизация breaker-open (CoreError::Unavailable) + WB backoff через max
21b726e feat(wb): отчёты конвертируются в Excel с русскими колонками (как у Ozon)
...
5. ~~**Автосохранение может восстанавливать test.documents** при провайдере WB~~ —
   **ЗАКРЫТО** (eeafba0): pending отчёта помечается провайдером; on_reports_loaded
   отбрасывает pending другого провайдера и выбирает первый элемент. Плюс починена
   гонка restore (если отчёты загружаются раньше состояния — теперь выбирается сразу).
6. **WB-отчёты сохраняются как JSON** — ~~потенциальный UX-хвост~~ **СДЕЛАНО**
   (21b726e): все WB-отчёты, кроме Documents API (настоящие файлы WB),
   конвертируются в xlsx с русскими колонками; пустой результат — честный
   JSON `[]`; ошибка конвертации — fallback на JSON.
7. **blocking-SQLite-in-async — осознанно отложено** (6bacad0): все вызовы
   Catalog в app-loop локальные индексированные (µs–ms на объёмах десктопа),
   медленный keyring уже в spawn_blocking (P2-5); обёртка ~26 call-sites в
   spawn_blocking пользы не даёт, риск регрессии — да. Вернуться, только если
   появятся тяжёлые каталог-запросы.
...
### «Не наши проблемы» Ozon — РАЗОБРАНЫ И РЕШЕНЫ (534509e, 2026-08-14 ночь)
Глубокий разбор с живыми пробами (`--log-level debug`, точные коды/тела) → решения:
- **`ozon.b2b_sales`** — create валиден, серверная трасса `getFinanceDocumentID →
  NotFound` = у аккаунта НЕТ документа B2B-продаж (не продаёт юрлицам). Теперь
  ошибка маппится в человекочитаемую: «нет документа реестра продаж юрлицам —
  кабинет не продаёт юрлицам (B2B)». Верифицировано живьём.
- **`ozon.postings`** — create успешен (code), генерация стабильно падает
  («Failed to build report», июль И февраль — не объём). Решение: ретраи
  (до 2 повторных create с паузой 30с, отменяемо; env MDWF_OZON_BUILD_RETRY_MS
  для тестов) → после исчерпания **фоллбэк**: отчёт собирается сам из
  /v3/posting/fbo/list (денормализация отправление×товар → xlsx, русские
  колонки). Живо: 3 попытки → фоллбэк → 666 строк / 44,6 КБ / 0 ошибок.
- **CLI-день** — period `YYYY-MM-DD` теперь → (date, date); раньше пустой
  filter → 400 «invalid date filter, expected processed_at or cutoff».
  Живо: postings за 2026-07-01 — ОК.
- **`/v2→/v3 fbo/list` (СРОЧНО)** — /v2 отключается 31.08.2026: авто-fill
  accrual_postings и fetch_posting_numbers мигрированы на /v3 (cursor,
  limit≤100). Живо: accrual июль — 80 КБ xlsx.
- **`warehouse_stock`** — в ошибку и описание добавлена подсказка «для FBO
  используйте „Аналитику по остаткам“».
- **WB claims 14 дней** — ограничение API (параметров дат нет), уже обойдено
  (два среза + локальный фильтр; стратегия — регулярный запуск для накопления
  истории в Архиве). Изменений не требуется.
...
59. **Смена формата содержимого виджета — проверь всех его потребителей.**
    QW1 («type_id спрятан») поменял заполнение комбо «Загрузки» с «id — имя»
    на только display_name — а `current_report_type()` продолжал парсить
    СТАРЫЙ формат (`split(" — ").next()`), тихо отдавая display_name как
    type_id. Все GUI-скачивания сломались МОЛЧА (тост с ошибкой гас за
    секунды). Поймано только живым клик-тестом спустя день — unit-тесты
    строк «проходят», text-verification OCR тоже (слова на месте).
    Правильно: (а) при смене контракта виджета grep-ить всех читателей
    (active_text/active_id/формат — как в уроке #55); (б) хранить id как
    ЗНАЧЕНИЕ (active_id), а не парсить текст; (в) критичный UX-флоу
    (скачивание) закрывать живым клик-тестом, не только скриншот-текстами.
    Харнес-уроки клика: popup-полосы — относительно якоря; якорь кнопки —
    по уникальному соседу строки (слова инструкции коллидеят с кнопками);
    состояние сложных форм проще предустановить в SQLite (value_json,
    кириллица — только .read UTF-8 файла, argv мутирует в CP866).
60. **UI кабинета опережает документацию — пути для юзера сверять с живым ЛК.**
    Строки «Соответствует разделу …» в docs.ozon.ru описывают СТАРЫЕ названия
    разделов кабинета: «Реестр продаж юр. лицам» → в ЛК «Продажи юрлицам»;
    «УПД по сделкам с юр. лицами → УПД по выкупленным товарам» → «Выкупы
    маркетплейсом»; «Отчёты о компенсациях/декомпенсациях» → единый раздел
    «Компенсации и прочие начисления». Всё, что адресует ПОЛЬЗОВАТЕЛЯ в
    интерфейс кабинета (наши cabinet_path), — первоисточник сам кабинет
    (скриншот юзера: разделы «Финансы → Документы» = Финансовая отчётность /
    УПД с доп. услугами / Отчёты о реализации / Выкупы маркетплейсом /
    Продажи юрлицам / Отчёты партнёров / Аналитические отчёты / Компенсации
    и прочие начисления / Обеспечительные платежи и штрафы / Счета на
    оплату / Акты сверки). Уточнение урока #22: «сверять с докой»
    недостаточно, когда дока описывает ЧУЖОЙ UI — там она обновляется
    медленнее самого продукта. Вторая партия скриншотов (структура ЛК
    2026-08): Финансы → [Документы (11 папок), Начисления и документы →
    Баланс/Доходы и расходы/История баланса]; Товары → Работа с товарами →
    Список товаров («Товары и цены» не существует); FBO → Заказы
    («Скачать CSV», не «Заказы со склада Ozon»); FBS → [Заказы с моих
    складов, Возвраты и отмены, Маркировка товаров, Обновить остатки на
    складах, Управление логистикой, Списанные товары] («Управления
    остатками» нет); Аналитика → Моя аналитика → Отчёты → Товары →
    Продажи со склада Ozon → [Управление остатками, Оборотная ведомость
    по товарам, Товары уценённые, Изменение SKU ID]. Breadcrumbs читаются
    кропом верхней полосы (200px) + vision — полный кадр мелкие крошки
    искажает («Мои продажи» вместо «Финансы»).
    Дополнение (15.08): официальный источник живой структуры — база знаний
    продавцов seller-edu.ozon.ru. Сам сайт и seller.ozon.ru за антиботом
    (бесконечный редирект ?__rr=), НО поисковики индексируют статьи —
    сниппеты дают точные пути («Финансы → Стоимость размещения → Скачать
    отчёт → По поставкам»). API-дока и база знаний расходятся в названиях —
    база знаний свежее (пишет «теперь скачивается в …» о переносах).
61. **Непроверенный путь деградирует тихо (CLI-исполнитель расписаний).**
    `mdwf schedule run` — путь фоновой задачи Windows — месяцами считался
    «рабочим» (задача не была установлена на dev-машине, живые тесты шли
    через GUI-Runner), а на деле: скачанные файлы ВЫБРАСЫВАЛИСЬ (только
    счётчик) и период не подставлялся из period_offset — т.е. фоновые
    выгрузки молча не происходили. Урок: если у фичи есть параллельные
    исполнители (GUI-Runner и CLI-исполнитель), ПРОВЕРЬ ЖИВЬЁМ каждый;
    «код выглядит так же» ≠ «код делает то же». Симптомов не было, потому
...
64. **MSYS/Git-Bash конвертирует switch-аргументы нативных exe в пути.**
    ...
    громкую ошибку.
65. **GtkCalendar: навигация по месяцам переносит выбранный номер дня.**
    После смены месяца выбранным остаётся тот же номер дня; клик по нему
    в новом месяце НЕ эмитит day-selected (день «не изменился») —
    «мёртвые» даты (жалоба юзера: 28 февраля после января). Фикс
    (make_date_picker, views/mod.rs): флаг navigating на next/prev-month,
    подавленный idle-перевыбор 1-го числа + GestureClick-fallback с
    фильтром координат (y>56 — ниже головы календаря, x>30).
66. **Обязательные к показу изображения — из gresource, не из icon theme.**
    Иконка приложения в MenuButton через `Image::from_icon_name` на
    машине без нужной темы рендерится «белым листом» (fallback темы).
    Тот же класс проблем — #39/#57 (иконки маркетплейсов, типов файлов).
    Паттерн: PNG в resources.gresource.xml → `Image::from_resource`
    (иконка приложения: alias `app.png`, pixel_size 22).
67. **Минимальный размер GTK-окна = максимум минимумов детей.**
    Окно не уменьшалось ниже ~1325×919: Label-колонки естественной
    ширины + незаскролленные вьюхи распирали. Диагностика —
    `measure(Orientation::Horizontal, -1)` по поддереву. Лечение:
    «тяжёлые» списки/таблицы в ScrolledWindow (гасит min-size),
    `width_chars == max_width_chars` у колонок-Label, настройки —
    подвкладки. Утверждённый юзером минимум: 840×590.
68. **SQLite: busy_timeout ставить ДО journal_mode=WAL.**
    Смена journal_mode требует краткого эксклюзивного лока; параллельное
    открытие каталога вторым процессом (GUI + задача планировщика;
    параллельные e2e-тесты CLI) без таймаута мгновенно падает
    «database is locked» (flake CI run 32078384696). Решение:
    `PRAGMA busy_timeout=5000` первым батчем (9a40616).
69. **Сохранение прокрутки при пересборке списка:** сохранить
    `vadjustment().value()` до очистки контейнера и восстановить в
    `idle_add_local_once` ПОСЛЕ отрисовки (синхронно значение ещё не
    пересчитано — прыжок в начало). Иначе удаление строки = сброс
    прокрутки (жалоба юзера на «Архив»; PRESERVE_SCROLL/W_SCROLL
    thread-locals в archive.rs).
70. **GtkColumnView (gtk4-rs 0.9) — API-грабли:** фабрика и selection
    передаются в конструкторы ПО ЗНАЧЕНИЮ (`ColumnViewColumn::new(Some(t),
    Some(factory))`, `ColumnView::new(Some(selection))` — передача по
    ссылке = E0277); `set_fixed_width(i32)` (НЕ Option; ширина 0/не задана
    = auto); `set_resizable(true)` даёт тянутые мышью границы. Ячейки —
    SignalListItemFactory (setup создаёт виджет, bind наполняет); list
    items ПЕРЕИСПОЛЬЗУЮТСЯ при прокрутке — bind обязан пересобирать
    контент (кнопки — rebuild children, иначе обработчики чужих данных).
    Модель строк без сабклассинга: `gio::ListStore` из
    `glib::BoxedAnyObject::new(T)` / `.borrow::<T>()`.
71. **PrintWindow НЕ снимает GTK4/GL-окна** — отдаёт пиксели перекрывающего
    окна или мусор (GSK-рендеринг вне DWM-редиректа). Для скриншотов окна:
    focus.ps1 (SetForegroundWindow) + CopyFromScreen по GetWindowRect.
72. **GDI+ может сломаться селективно по диску/формату:** Save в PNG на D:
    падал «A generic error occurred in GDI+» при живом C: (и BMP на C:
    работал) — дело не в коде и не в месте на диске. Обход в репо:
    `scripts/gui-test/shot_bmp.ps1` (снимок в BMP на C:) +
    `bmp2png.py` (stdlib zlib, конвертация). Перезагрузка, вероятно,
    чинит; обход остаётся как страховка.
73. **`cmd /C start "" "<url>"`: кавычки обязательны при `&` в URL.**
    std::process квотирует только аргументы с пробелами — URL с
    амперсандами без явных кавычек режется cmd на команды (`tab=…` ->
    «not recognized»). Решение: `CommandExt::raw_arg` с самодельными
    кавычками `format!("\"{url}\"")` + пустой title `""` (первый
    закавыченный аргумент start — заголовок окна, не цель).
74. **Справка — часть контракта UI:** при смене терминологии/контролов
    обновлять ВСЕ поверхности (вкладка «Справка», «?»-подсказки вкладок,
    FAQ, названия кнопок в текстах). Найдено: справка годами упоминала
    «неделю» в интервалах после её удаления из виджета; «после загрузки —
    кнопка Открыть папку» при фактических двух. Чек-лист сверки: термины
    интервалов, кнопки действий, ЛК-ссылки, колонки/иконки.

**Чат 2026-08-14 (закрытие остатков бэклога; efcab98, eeafba0):**
- **Единая иконка «Обновить»** (efcab98): кнопка «↻ Обновить» в «Отчётах»
  была пропущена при миграции на symbolic — теперь обе через
  icon_button(view-refresh-symbolic).
- **Три GUI-фикса** (eeafba0): (1) Pango-markup: «<папка загрузок>»/
  «<папку>» в справке ломали set_markup (2 Gtk-WARNING на каждом старте)
  → экранированы, живая проверка 0 warning; (2) гонка restore комбо
  «Загрузки» (LoadReports раньше LoadDownloadState → pending не
  потреблялся) → restore сразу зовёт select_report_by_type; (3)
  stale-state guard (§11-5 закрыт): pending помечается провайдером,
  чужой отбрасывается. Живая проверка: активен ozon + стейт wildberries →
  комбо первый Ozon-отчёт.
- **Клик-тест Журнала/Планировщика** (§14-5 закрыт): расписание js_smoke
  «+2 минуты» (CLI schedule add) → карточка в «Расписаниях» → фоновый
  автозапуск (цикл 60с) → запись в «Журнале» (~50с после due). Нюанс CLI:
  `schedule delete <NAME>` — name позиционный, подтверждение по stdin.
- **Чистая машина по финальному бандлу** (19:34): test-clean-env.sh —
  стартует без MSYS2/GTK-env, has_mdwf=true, миграция БД 0→4;
  test-installed.sh — postinstall exit 0, запуск в пути с пробелом
  (exit 124 = ОК), loaders.cache относительные пути (relocatable).
- Финальный инсталлер: `MDWFSetup-1.4.0.exe` 19:34 — включает всё.

**Чат 2026-08-15 (день→вечер: журнал-пакет, фикс CLI-исполнителя, cabinet-пути,
метаданные+релиз v1.4.0, self-test драйвер, версионирование; ce29251…27e7216):**
- **Хэндофф-контекст**: чат начался с чтения этого файла; контекст деградировал
  к вечеру — этот снимок подготовлен для следующего чата.
- **MDWF_START_VIEW** (ce29251) + бес кликовый живой прогон фоллбэка postings
  через расписание fb_smoke (Журнал получил note-строку, файл = CLI-прогону).
- **Журнал-пакет** (9a698e9, d54f109): персистенс (v5) + origin (v6,
  mdwf_core::LogOrigin) + названия/периоды (describe → core::journal) +
  CLI download пишет журнал. Живо: CLI-выгрузка, запуск расписания из CLI,
  GUI-рестарт с историей — все записи с [источником].
- **Фикс CliJobExecutor** (06b3b47): период из offset + persist (раньше файлы
  выбрасывались!) + `schedule run --by-task` (wintasks ставит флаг). Урок #61.
- **Cabinet-пути**: все 21 Ozon сверены (юзер по живому ЛК: Закрывающие/
  Справочные, «Моя аналитика»; база знаний seller-edu через сниппеты:
  placement_* → Финансы, returns → Аналитика). Таблица всех 21+13 путей
  выдана юзеру для сверки WB.
- **Метаданные + релиз**: LICENSE×2/NOTICE/CHANGELOG/SECURITY/CONTRIBUTING,
  Cargo repository/автор, README-факты; тег v1.4.0 + Release с инсталлером
  (10:15, clean-env проверен); topics+описание репо.
- **--self-test** (0070ee5): headless event-level драйвер (isolated=true по
  умолчанию — <сценарий>.data/), сценарии smoke + live_ozon_balance PASS;
  обёртка scripts/self-test.sh (грабля: env.sh делает exec "$@" при
  позиционных аргументах — чистить `set --`). Урок #62.
- **Версионирование** (3b6c462): 1.5.0-unreleased; единый источник —
  Cargo.toml (ISCC /DMyAppVersion, #ifndef-guard в .iss); политика в
  CONTRIBUTING. Cargo.lock тоже должен коммититься при bump (поймали).
- Итоговое состояние: 23 сюита зелёные, clippy 0, всё запушено.

**Чат 2026-08-14/15 (вечер→ночь: двойные клики, интервал, N/A-решения, ЛК-пути; 962d66f…4ea596c):**
- **Двойной клик — редактирование** (`962d66f`, `ecc28b4`): строка профиля
  в «Магазине» (ListView — штатный `connect_activate` работает) и карточка
  расписания в «Планировщике» (`GestureClick` с `n_press>=2` — у ListBoxRow
  сигнал `activate` в GTK 4.20/MinGW на двойной клик НЕ срабатывает, eprintln
  показал ноль вызовов) открывают тот же диалог правки, что и кнопки «✎».
- **Интервал-виджет** (`c57ab9e`): «Неделя» убрана, «Полугодие»
  (`half_range(y,1|2)`) добавлена перед «Годом». Под полем дат — живое
  описание периода `describe_range(from,to)` (`views/mod.rs`, +7 тестов):
  «2024 год» / «январь 2025» / «второе полугодие 2024» / «3 квартал 2025» /
  «23 января 2026» / иначе «с 04.03.2025 по 06.03.2025». Считается из самих
  дат → одинаково для ручного ввода и виджета. Живая vision-верификация.
- **N/A-решения** (`534509e`, разбор `482b8c4`, детали §11): b2b_sales —
  friendly-error; postings — retry create×3 (пауза 30с, env
  `MDWF_OZON_BUILD_RETRY_MS` для тестов, отменяемо через tokio::select) →
  **фоллбэк**: своя сборка xlsx из `/v3/posting/fbo/list`
  (`sheet_postings_from_fbo_list`, денормализация отправление×товар);
  `fetch_posting_numbers` → /v3; CLI `--period YYYY-MM-DD` → (date,date).
- **ЛК-пути отчётов** (`391f8af` + фикс `4ea596c`): `ReportDescriptor.
  cabinet_path` (+builder `with_cabinet_path`, WB тоже — 13 дескрипторов).
  Поверхности: «Отчёты» (dim-строка «ЛК: …» + tooltip), «Загрузка»
  (инфо-панель «В ЛК: …»), «Справка» (раздел 8a «Отчёты и их расположение
  в личном кабинете» — генерируется из `all_report_descriptors()` обоих
  провайдеров, тест-страж формата). **Названия — из ЖИВОГО кабинета**
  (скриншот юзера; список разделов в уроке #60), дока переведена на
  вспомогательную роль. Пути вне «Финансы → Документы» пока по доке —
  скриншотами не покрыты, при расхождении юзер пришлёт скриншот.
  Плюс `DownloadedFile.note` — информирование о фоллбэке postings:
  CLI stdout, GUI Журнал «— внимание: {note}», progress-сообщения.
  **Вторая партия скриншотов** (`2ab6af9`): юзер прислал 5 скриншотов
  (Баланс/Товары/FBO-Заказы/FBS-остатки/Аналитика-уценка) — исправлены
  ещё 8 путей: products, postings, discounted, analytics_stocks,
  analytics_turnover, warehouse_stock, balance, cash_flow. Методика:
  кроп верхней полосы 200px → vision (полный кадр крошки искажает).
  По доке остались только placement_* и returns (не покрыты скриншотами).
- **build-setup.cmd из PowerShell/двойного клика** (`b0d7ab5`): (1) bash не
  в PATH → cmd сам находит bash.exe (Git Bash/MSYS2); (2) не-login bash не
  имеет /usr/bin → prepend dir bash'а; (3) Windows-путь «D:/…» в bash-PATH
  не работает → `cygpath -u`; (4) cargo → `%USERPROFILE%\.cargo\bin`.
  Проверен end-to-end из PowerShell: `[OK] Done`.
- Инсталлер пересобран после всего: `MDWFSetup-1.4.0.exe` (15 авг 01:47).

**Чат 2026-08-15 (ночь→утро: проверки бандла, GUI-фоллбэк postings, журнал
в БД + источники + названия/периоды, фикс CLI-исполнителя; ce29251…06b3b47):**
- **test-clean-env/test-installed** на бандле 02:06 — зелёные (см. §0-1).
- **MDWF_START_VIEW** (ce29251): env-хук стартовой вкладки — скриншот
  нужной вкладки без кликов по живому столу (урок #51). Живой прогон
  фоллбэка postings через расписание fb_smoke — Журнал получил note-строку,
  файл байт-в-байт с CLI.
- **Журнал персистится** (9a698e9): таблица journal (v5, кап 500 =
  JOURNAL_KEEP), LogEntry.created_at DateTime<Utc>, LoadJournal при старте,
  ClearJournal («Очистить» = БД + лента), +2 теста storage.
- **Источники и названия** (d54f109): journal.origin (v6) + LogOrigin
  (core::journal, 6 вариантов словаря), describe_range/report_period →
  core (общие для GUI/CLI, 7 тестов переехали), subject-хелперы,
  чип [источник] в ленте, CLI download пишет журнал. Живо верифицировано.
- **Фикс CliJobExecutor** (06b3b47): период из offset + persist через
...
**Чат 2026-08-18/20 (фиксы по жалобам + релиз 1.6.0 + ствол 1.7.0; af963e5…3f43ce8):**
Волна правок по сигналам юзера, каждая закрыта отдельно (тесты/clippy
двух тулчейнов/визуальная верификация). Подробности в §0-3; неочевидное:
- **Таблица юзера = источник правды для Ozon**: файл в корне репо
  `Отчёты_Ozon_методы_и_пути_ЛК.xlsx` юзер дорабатывает живьём (три
  итерации за сессию); сверка — python-скриптом в чате (названия/методы/
  типы/пути/ссылки, нормализация пробелов и «→»); после каждой правки
  юзера — точечные Edit в REPORT_DEFS.
- **«Оборачиваемость» vs «Оборотная ведомость»**: переименовал по таблице
  → юзер дважды подтвердил «это Оборачиваемость товара» → откат
  (af963e5) → юзер поправил ячейку в таблице. Урок: при конфликте
  «таблица vs слова юзера» — спрашивать словами, не молча выбирать.
- **Релиз 1.6.0**: CHANGELOG-однострочник (паттерн 1.5.0, БЕЗ нового
  [Unreleased] — секция открывается при первом пункте нового цикла);
  build-setup.sh из Git Bash (вариант B хэндоффа); test-clean-env
  зелёный (схема 6→7 на живой миграции); тег на релизном коммите;
  сначала дождались зелёного CI кодовых коммитов.
- **GDI+ на dev-машине сломался** (PNG на D: → «общая ошибка», на C:
  работает): обход scripts/gui-test/shot_bmp.ps1 + bmp2png.py (урок #72).
- **Визуальная верификация при юзере за машиной**: MDWF_START_VIEW +
  focus.ps1 (кратко на передний план) + CopyFromScreen; PrintWindow для
  GTK/GL-окон НЕ работает (урок #71).
- **e2e-флэйк «database is locked»** повторился 1 раз на CI (rerun зелёный):
  residual урока #68 — кандидат на укрепление Catalog::open.
- **CI отменяет ран предыдущего пуша** (concurrency cancel-in-progress) —
  successive-пуши: следить только за последним раном, отмена старого
  НЕ ошибка.

**Чат 2026-08-16—17 (UX-волна юзера; 4fe333d…9a40616, 12 коммитов):**
Серия мелких правок по жалобам юзера — каждая закрыта отдельно
(тесты/clippy двух тулчейнов/визуальная верификация скриншотами, где
UI). Подробности в §0-3; здесь — неочевидное:
- **Сводная таблица отчётов Ozon**: по просьбе юзера выдана единая
  xlsx-таблица всех 21 методов (метод API / название / путь ЛК / ссылка
  браузера) — один лист. Юзер ДОРАБОТАЛ её (колонка «Ссылка для
  браузера», 10 исправленных путей) → таблица стала источником правды
  для cabinet_url/путей (981986d). Файл юзера не в репо.
- **Спецификация интервального виджета**: по просьбе юзера выдано полное
  описание виджета выбора периода для LLM-агента (внешнего) — как
  сообщение чата, не артефакт репо.
- **Версионирование** (вопросы юзера, код не менялся): MINOR на первой
  фиче цикла (ствол 1.6.0-unreleased с `b8f7a73`), PATCH (1.6.1…) —
  только hotfix'ы после выхода базовой. CONTRIBUTING формулирует PATCH
  чуть двусмысленно — можно уточнить формулировку при случае.
- **Методология визуальной проверки GUI** (без кликов при юзере за
  машиной): запуск с `MDWF_START_VIEW=<view>` → shot.ps1 (DPI-aware) →
  кроп по GetWindowRect (System.Drawing) → Read → vision-модель
  (analyze_image) для подтверждения элементов; до/после —
  `taskkill //IM mdwf-gui.exe //F` (иначе os error 5 на билд).
- **Итог**: 235+ тестов зелёные, clippy 1.90/1.97 чист, CI зелёный
  (32079574415). Готов релиз 1.6.0 — по команде юзера.
...
**Чат 2026-08-14 (продолжение: WB→Excel + хвосты + per-user инсталлер; 21b726e, 6bacad0):**
По выбору пользователя закрыты 4 направления:
- **WB→Excel** (`21b726e`): новый `crates/providers/wildberries/src/xlsx.rs`.
  Все WB-отчёты (кроме Documents API — настоящие файлы WB) теперь .xlsx:
  курируемые упорядоченные колонки с русскими именами (detailed 36 полей,
  orders/sales/claims/удержания/антифрод/приёмка/реестр/эквайринг/баланс)
  + **авто-добавление всех прочих полей ответа** с raw-именами — при изменении
  схемы WB данные не теряются молча (отличие от жёстких списков Ozon).
  Числа — числами, bool → да/нет, вложенные объекты → компактный JSON.
  Пустой результат — честный JSON `[]`; ошибка конвертации — fallback на
  JSON + warn (данные не теряются). Живая проверка: list → 7 строк;
  detailed → 3218 строк (925 КБ), заголовки русские. +7 unit-тестов,
  интеграционные обновлены на ZIP-magic + тест пустого результата.
- **Типизация breaker-open** (`6bacad0`): `CoreError::Unavailable(String)` —
  transient (в `is_transient()`), НЕ «баг кода» как прежний Internal.
  health_check теперь честно даёт Degraded (был Down); CLI → ApiError;
  GUI friendly_error — «запросы временно приостановлены защитой от
  перегрузки, подождите ~5 минут». Тест breaker +assert Degraded.
- **WB RateLimiter.backoff через max** (`6bacad0`): два штрафа подряд
  (30с затем 1с) больше не сокращают ожидание (аналог Ozon P0-3) + тест.
- **blocking-SQLite-in-async**: осознанно отложено с обоснованием (см. §11-7).
- **Инсталлер per-user** (этот же пакет): `PrivilegesRequired=lowest` +
  `OverridesAllowed=commandline` → установка в %LOCALAPPDATA%\Programs\MDWF
  БЕЗ UAC (у MDWF нет системных компонентов); /ALLUSERS — явная опция.
  ⚠️ Апгрейд поверх старой админ-установки не подхватится (HKLM vs HKCU) —
  старую копию удалить вручную. Финальная пересборка инсталлера включает
  ВСЁ перечисленное.
- **GUI smoke (фаза-1, без кликов)**: свежий debug-бинарь запущен живьём —
  окно рендерится, sidebar-секции на месте, «Отчётов: 21», профили
  oz_prof1/wb_prof в списке — новый код GUI не сломал (скриншот
  dist/_smoke/wb1.png; юзер был за машиной — клики не выполнялись, фаза-2
  отложена). Урок #56 соблюдён (явный cargo build перед проверкой).

**Чат 2026-08-14 (клик-тест WB: пойман P0-регресс GUI; коммит 8c11fe8):**
Юзер освободил машину → полная клик-фаза (OCR+SendInput, scripts/gui-test/).
Итеративно добивалась (v1→v5, дистрибутив dist/_gt/wb_smoke*.sh):
- ✓ Магазин: комбо «Маркетплейс» → Wildberries, профиль wb_prof (клик по
  СТРОКЕ профиля в списке НЕ переключает магазин — только комбо).
- ✓ Отчёты: WB-имена в списке (Эквайринг/приёмки/Баланс).
- ✓ Отчёты → клик по отчёту → авто-переход в «Загрузку» с предвыбором
  (set_pending_report) — самый надёжный путь выбора отчёта (combo-popup
  флейчит: popup не всегда переживает focus-трюк скриншота).
- ✓ «Скачать по периоду» → живое скачивание WB (3 POST) → **xlsx 925 132
  байт — байт-в-байт совпал с CLI-выгрузкой** → кнопки «Открыть» появились.
  Магазин возвращён на oz_prof1.
- **НАЙДЕН P0-РЕГРЕСС** (8c11fe8, сломан с QW1 «type_id спрятан»):
  `current_report_type()` парсил старый формат комбо «id — имя»
  (`split(" — ").next()`), а комбо с QW1 показывает только display_name →
  в `UiCommand::Download` уходило человеческое имя → provider.report() →
  ReportTypeNotSupported → **ВСЕ скачивания через GUI молча не работали
  для обоих провайдеров** (тост гас быстро). Никем не поймано, потому что
  после QW1 GUI-тесты были только text-verification. Фикс: type_id из
  REPORTS по active-индексу. Заодно починен persist download_screen
  (хранит type_id; restore комбо работает).
- **Харнесс-уроки** (урок #59): полосы popup-поиска — только ОТНОСИТЕЛЬНО
  якоря (окно двигается между запусками); «первое слово» вредит — текст
  инструкции содержит те же слова, что и кнопки («Скачать по периоду» в
  подсказке!) → якорь по строке уникального соседа («Отмена»); ввод дат
  SendInput-ом глючит фокусом — состояние «Загрузки» проще предустановить
  в SQLite ui_state (value_json; кириллица через argv sqlite3 мутирует в
  CP866 — только `.read file.sql` UTF-8); restore комбо download_screen
  гоняет с загрузкой списка отчётов (даты восстанавливаются, комбо —
  флапает; не чинил — путь через «Отчёты» надёжнее).
...
1. ~~**Аудит WB API**~~ — ✅ ВЫПОЛНЕНО (2026-08-14): сверка со спекой + живой
   прогон 12/12, фиксы см. §14.
2. ~~**Серверные ошибки Ozon**~~ — ✅ РЕШЕНЫ (534509e, вечер 14.08): b2b_sales —
   friendly-error («нет B2B-документа — кабинет не продаёт юрлицам»);
   postings — retry create×3 + **фоллбэк** (своя сборка xlsx из /v3 fbo/list,
   note информирует юзера). Детали §11.
3. ~~**FBS-поддержка для auto-fill**~~ — ✅ СДЕЛАНО (P1-e): `/v4/posting/fbs/list`
   (v3 отключается 31.08.2026), auto-fill объединяет FBO+FBS. Пункт был устаревшим.
4. **Доработки из известных проблем** (секция 11): крупные закрыты
   (async-отчёты WB, max_parallel_jobs, Журнал/Планировщик).
5. ~~**GUI-клик-тест Журнала/Планировщика**~~ — ✅ ВЫПОЛНЕНО (2026-08-14):
   расписание js_smoke «через 2 минуты» (CLI) → карточка в «Расписаниях» →
   фоновый автозапуск → запись в «Журнале».
6. ✅ **Проверка `setup.exe` на чистой Windows** — ВЫПОЛНЕНО (бандл полон,
   инсталлер/deинсталлятор работают, relocatable). ✅ per-user без админа —
   СДЕЛАНО (60361bb, PrivilegesRequired=lowest).
