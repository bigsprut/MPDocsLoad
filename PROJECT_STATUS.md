...
6. ✅ **Офлайн-режим навигации по скачанным документам с фильтрами** — СДЕЛАНО.
   ...
   report_type в БД) / период (combo Месяц+Год, `(все)`=без фильтра; период — поле
   `downloads.period`, YYYY-MM; у WB-документов оно NULL). Колонки: Профиль | Отчёт |
   Период | Формат | Размер | Скачан | Действия. Действия per-row: 📂 Открыть файл,
   📁 Открыть папку, 📋 Копировать путь (недеструктивно, без удаления).
...
```
<П.6/2> feat(gui): вкладка «Архив» офлайн-навигации + фильтры + действия (П.6)
...
**В этом чате завершены**: П.5 (иконка типа файла, эмодзи в списке документов WB) и
П.6 (вкладка «Архив» — офлайн-навигация по скачанным документам с фильтрами профиль/
отчёт/период + действия открыть файл/папку/копировать путь). Все 6 пунктов бэклога закрыты.
Все изменения запушены (последний коммит — см. секцию 6, поиск «П.6»).
...
1. Вкладка **«Архив»**: при старте показывает все скачанные файлы (если они есть в БД).
   ...
   Кнопки 📂/📁/📋 в строках. Архив НЕ требует токенов/сети — только локальный SQLite.
