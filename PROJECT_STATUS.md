...
- **Инсталлер актуален:** `installer/Output/MDWFSetup-1.4.0.exe`
  (15 авг, 02:06) — включает весь вечер 14–15 авг (двойные клики,
  интервал-виджет, N/A-решения, ЛК-пути обеими партиями, build-setup).
...
- **Инсталлер 02:06 ОТСТАЁТ** от HEAD 06b3b47 (не включает журнал-пакет
  этой ночи) — перед выдачей пересобрать `bash scripts/build-setup.sh`.
- **Ближайшие задачи:** крупных нет. 15.08 закрыты и последние два
  cabinet-пути (d2ffcd5): placement_* → «Финансы → Стоимость размещения
  (по товарам/по поставкам)», returns → «Аналитика → Моя аналитика → Отчёты → Возвраты» (уровень
  «Моя аналитика» уточнён юзером) —
  по официальной базе знаний продавцов (seller-edu.ozon.ru, доступ через
  поисковые сниппеты — сам сайт за антиботом). Все 21 ЛК-пути сверены.
  Инсталлер перед выдачей пересобрать (отстаёт от HEAD).
- **Живые API-тесты разрешены пользователем** (профили `oz_prof1` +
  `wb_prof`, ключи валидны).
- Ключевые уроки нарастающим итогом: **#53-60** (добавлен #60: UI кабинета
  опережает документацию — пути для юзера сверять с живым ЛК).
