...
- **HEAD:** `4ea596c` (master = origin/master, рабочее дерево чистое).
...
- **Инсталлер актуален:** `installer/Output/MDWFSetup-1.4.0.exe`
  (15 авг, 01:47) — включает весь вечер 14–15 авг (двойные клики,
  интервал-виджет, N/A-решения, ЛК-пути, build-setup правки).
- **Сделано последним пакетом (вечер 2026-08-14 → ночь 15-го):**
  ...
  4. **ЛК-пути отчётов в GUI** (`391f8af` + фикс `4ea596c`): три
     ...
     взяты из ЖИВОГО кабинета (скриншот юзера): docs.ozon.ru называет их
     по-старому («Реестр продаж юр. лицам» = «Продажи юрлицам», «УПД по
     выкупленным товарам» = «Выкупы маркетплейсом», «Отчёты о
     компенсациях/декомпенсациях» = «Компенсации и прочие начисления»).
     Урок #60. Плюс note-информирование о фоллбэке postings (Журнал
     «— внимание: …»).
...
- **Ближайшие задачи:** крупных не осталось. Опционально: живой GUI-прогон
  ...
  сверка ЛК-путей ВНЕ «Финансы → Документы» (FBO/FBS/Аналитика/Товары —
