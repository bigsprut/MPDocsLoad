...
- **HEAD:** см. `git log` (master = origin/master, рабочее дерево чистое; все
  наработки 2026-08-14 запушены: UX-пакет, P0-P2 фиксы, symbolic-иконки,
  REPORT_DEFS 4→1, sidebar, живой WB-аудит, **WB→Excel**, хвосты, per-user
  инсталлер).
...
- **Сделано этим чатом (2026-08-14, вечер):**
  1. **Инсталлер пересобран** после всего: UX-пакет → WB-фиксы → **WB→Excel
     + типизация ошибок + per-user без админа** (`PrivilegesRequired=lowest`,
     установка в %LOCALAPPDATA%\Programs, UAC не запрашивается; /ALLUSERS —
     опция командной строки). Выдача пользователю актуальна.
  2. **Живой WB-аудит ЗАВЕРШЁН**: сверка с OpenAPI-спекой (зеркало
     eslazarev/wildberries-sdk) + baseline + фиксы + повторный живой прогон —
     **12/12 отчётов OK** (detailed: 1000 → 3217 строк; claims is_archive;
     acceptance poll→download). Коммит `06b04cc`.
  3. **WB→Excel**: все WB-отчёты (кроме Documents API) конвертируются в
     xlsx с русскими колонками; курируемые колонки + авто-добавление прочих
     полей (данные не теряются при изменении схемы WB). Коммит `21b726e`.
  4. **Хвосты**: `CoreError::Unavailable` для breaker-open (transient,
     health_check → Degraded; CLI ApiError; GUI — внятная подсказка);
     WB `RateLimiter.backoff` через max; blocking-SQLite-in-async —
     осознанно отложено (обоснование в §11). Коммит `6bacad0`.
- **Ближайшие задачи:** крупных не осталось. Опционально: живой GUI
  smoke-тест WB (CLI-прогон был; клик-тест — по готовности пользователя),
  мелкие UX-идеи.
