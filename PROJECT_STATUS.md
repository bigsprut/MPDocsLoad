...
отчёт/период + действия открыть файл/папку/копировать путь), фикс П.6 (фильтр периода
для WB-документов + `{doc_date}`), и **4 доп. фичи сверх бэклога**: persist фильтров
Архива, CLI `archive list`, удаление записи+файла, индекс `idx_downloads_filter`.
Все 6 пунктов бэклога закрыты + 4 доп. улучшения. Все изменения запушены (последний
коммит `a3b571c`).
...
**Возможные дальнейшие улучшения (доп. фичи сверх исходного бэклога — СДЕЛАНО):**
- ✅ **Persist фильтров Архива** в `ui_state["archive_screen"]` — автосохранение при
  смене combo (профиль/отчёт/месяц/год), восстановление при старте (как DownloadState).
  Защита от петли через RESTORING-флаг; отложенный restore профиля/отчёта через
  PENDING (combos заполняются асинхронно). Список при старте строится сразу с
  восстановленным фильтром, без «мигания» всеми записями.
- ✅ **CLI-аналог Архива** — `mdwf-cli archive list [--profile NAME] [--report TYPE]
  [--period YYYY-MM]`. Офлайн-просмотр (без API), таблица Профиль|Отчёт|Период|
  Формат|Размер|Скачан|Путь. Резолв профиля + period_to_range (пересечение диапазонов).
- ✅ **Удаление записи+файла** в Архиве — кнопка 🗑 в каждой строке, через
  `adw::MessageDialog` с красной кнопкой (`ResponseAppearance::Destructive`).
  `Catalog::delete_download(id)` + `std::fs::remove_file`; ошибка файла НЕ блокирует
  удаление записи (NotFound — не ошибка). Refresh списка после удаления.
- ✅ **Индекс** `idx_downloads_filter` на `(profile_id, report_type)` для
  `list_downloads_filtered` (предикаты `=` селективны; period не включён —
  CASE-выражение не sargable). `CREATE INDEX IF NOT EXISTS`, без миграции.

**Проверить вживую доп. фичи:** CLI `cargo run -p mdwf-cli -- archive list` (таблица);
GUI Архив — сменить фильтры, перезапустить → восстанавливается; кнопка 🗑 → диалог →
запись пропадает, файл стирается.
